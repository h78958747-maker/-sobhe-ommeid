...
const App: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      playError();
      // If error is a safety error, use key, otherwise use the specific error message if possible
      ...
      <div className="min-h-screen bg-black text-white relative overflow-hidden flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center p-6 relative z-10 animate-reveal">
          <div className="w-full max-w-4xl space-y-12">
            ...
            <div className="bg-white/5 backdrop-blur-3xl rounded-[4rem] p-2 border border-white/10 shadow-glass-heavy transform transition-all hover:scale-[1.01]">
              <ImageUpload 
                title={t.uploadTitle}
                selectedImage={null}
      ...
      <div className="min-h-screen bg-[#020202] text-white relative overflow-hidden flex flex-col">
        ...
        <header className="relative z-50 p-6 flex justify-between items-center border-b border-white/10 backdrop-blur-3xl bg-black/60 shadow-glass">
           <div className="flex items-center gap-4 cursor-pointer group" onClick={reset}>
             <img src={APP_LOGO} className="w-10 h-10 rounded-xl shadow-neon-blue border border-white/20 group-hover:rotate-6 transition-transform" alt="logo" />
           ...
           <div className="flex items-center gap-4">
             <button onClick={reset} className="px-6 py-2.5 text-[10px] font-black uppercase tracking-widest bg-white/5 border border-white/10 rounded-xl hover:bg-red-500/10 hover:text-red-500 transition-all backdrop-blur-xl">
        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden relative z-10">
          ...
          <section className="lg:col-span-3 border-r border-white/10 p-8 space-y-10 overflow-y-auto bg-white/5 backdrop-blur-2xl custom-scrollbar animate-slide-in-left">
             <div className="space-y-8">
                <div className="flex items-center gap-3 text-studio-gold">
                   <svg className="w-5 h-5 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M10.5 6a7.5 7.5 0 107.5 7.5h-7.5V6z" strokeWidth={2}/><path d="M13.5 10.5H21A7.5 7.5 0 0013.5 3v7.5z" strokeWidth={2}/></svg>
                ...
                <div className="space-y-4 bg-white/5 p-5 rounded-3xl border border-white/5">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {(['AUTO', '1:1', '4:3', '16:9', '9:16', '3:4'] as AspectRatio[]).map(ratio => (
                      <button key={ratio} onClick={() => { playClick(); setAspectRatio(ratio); }} className={`py-3 text-[9px] font-black border rounded-xl transition-all ${aspectRatio === ratio ? 'border-studio-neon text-studio-neon bg-studio-neon/20 shadow-neon-blue/20' : 'border-white/5 bg-white/5 text-gray-500 hover:border-white/20'}`}>
                ...
                {/* Lighting Styles with Icy Icons */}
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {(Object.keys(LIGHTING_STYLES) as LightingIntensity[]).map(l => (
                      <button key={l} onClick={() => { playClick(); setLighting(l); }} className={`p-5 border rounded-2xl flex flex-col items-center gap-3 transition-all ${lighting === l ? 'border-studio-neon bg-studio-neon/20 text-studio-neon shadow-neon-blue' : 'border-white/5 bg-white/5 text-gray-400 hover:border-white/20'}`}>
                ...
                <div className="space-y-4">
                   ...
                   <div className="space-y-2">
                      {(Object.keys(COLOR_GRADING_STYLES) as ColorGradingStyle[]).map(style => (
                        <button key={style} onClick={() => { playClick(); setColorGrading(style); }} className={`w-full py-4 px-5 flex items-center gap-4 text-[9px] font-black border rounded-xl transition-all ${colorGrading === style ? 'border-studio-neon text-studio-neon bg-studio-neon/20' : 'border-white/5 bg-white/5 text-gray-500 hover:border-white/20'}`}>
          ...
          <section className="lg:col-span-5 p-12 flex flex-col h-full items-center justify-center space-y-12 animate-reveal">
             <div className="w-full h-full max-h-[650px] relative rounded-[4rem] overflow-hidden shadow-glass-heavy border border-white/10 bg-black/40 p-6 group/monitor">
                ...
                <img src={selectedImage} alt="Monitor" className="w-full h-full object-contain rounded-[3rem] transition-all duration-1000 group-hover/monitor:scale-110 group-hover/monitor:rotate-1" />
             ...
             <div className="w-full space-y-6">
                <div className="flex flex-wrap justify-center gap-4">
                   {PROMPT_SUGGESTIONS.map(s => (
                     <button key={s.id} onClick={() => { playClick(); setDescription(s.prompt); }} className={`px-10 py-4 rounded-full border text-[10px] font-black tracking-widest transition-all hover:scale-110 active:scale-90 flex items-center gap-3 ${description === s.prompt ? 'border-studio-neon text-studio-neon bg-studio-neon/20 shadow-neon-blue' : 'border-white/5 bg-white/5 text-gray-500 hover:border-white/20'}`}>
      ...
      <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
        ...
        <div className="max-w-xl w-full bg-red-500/5 border border-red-500/20 rounded-[4rem] p-16 text-center space-y-10 relative z-10 backdrop-blur-3xl animate-reveal overflow-hidden shadow-glass-heavy">
          ...
          <div className="w-24 h-24 bg-red-500/20 rounded-full flex items-center justify-center mx-auto shadow-2xl">
            <svg className="w-12 h-12 text-red-500 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="space-y-4">
             <h2 className="text-3xl font-black uppercase tracking-tighter text-red-500">
               {isSafety ? t.ERR_SAFETY : t.ERR_GENERIC}
             </h2>
             <p className="text-gray-400 text-sm leading-relaxed whitespace-pre-wrap">
               {isSafety ? t.ERR_SAFETY_DESC : (processing.error === 'ERR_GENERIC' ? t.ERR_GENERIC_DESC : processing.error)}
             </p>
          </div>
    ...
    return (
      <div className="min-h-screen bg-black text-white relative flex flex-col">
        ...
        <header className="relative z-50 p-6 flex justify-between items-center bg-black/60 backdrop-blur-3xl border-b border-white/10 shadow-glass">
           <div className="flex items-center gap-4 cursor-pointer group" onClick={reset}>
             <img src={APP_LOGO} className="w-10 h-10 rounded-xl shadow-neon-blue border border-white/20" alt="logo" />
             <h1 className="text-sm font-black uppercase tracking-widest">{t.appTitle}</h1>
           </div>
           <div className="flex gap-4">
              <Button variant="secondary" className="px-8 h-12 rounded-xl text-[10px] tracking-widest backdrop-blur-2xl border-white/10" onClick={reset}>{t.newGeneration}</Button>
              <Button variant="secondary" className="px-8 h-12 rounded-xl text-[10px] tracking-widest backdrop-blur-2xl border-white/10" onClick={() => { playClick(); setShowEditor(true); }}>{t.viewEdit}</Button>
              ...
              {comparisonMode ? (
                ...
                <div className="relative aspect-auto max-h-[80vh] flex items-center justify-center p-8">
                   ...
                   <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover/master:opacity-100 transition-all duration-700 flex items-end justify-center pb-20">
                      <Button variant="primary" onClick={() => { playClick(); setComparisonMode(true); }} className="px-20 h-20 rounded-full text-[13px] font-black tracking-[0.2em] shadow-neon-blue backdrop-blur-3xl bg-black/40 border-white/20 hover:scale-110">
                        {t.viewCompare}
                      </Button>
                   </div>
                </div>
